...
def load_data(w2v_model): #导入数据，对数据进行处理
...
def train(w2v_model):
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            # Define Training procedure #确定培训程序
            global_step = tf.Variable(0, name="global_step", trainable=False)
            optimizer = tf.train.AdamOptimizer(1e-3)
            grads_and_vars = optimizer.compute_gradients(cnn.loss)
            train_op = optimizer.apply_gradients(grads_and_vars, global_step=global_step)

            # Keep track of gradient values and sparsity (optional) #跟踪渐变值和稀疏度（可选）
            grad_summaries = []
            ...
            # Summaries for loss and accuracy #损失和准确性总结
            ...
            # Checkpoint directory. Tensorflow assumes this directory already exists so we need to create it #检查点目录。Tensorflow假设这个目录已经存在，所以我们需要创建它
            ...
            # Training loop. For each batch... 训练循环。对于每个批次。。。
